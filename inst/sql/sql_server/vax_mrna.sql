...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (37003431, 37003516, 37003436, 37003435, 37003518, 37003434, 35895095, 35895096, 35891709, 35895097, 35891603, 37003432, 37003517, 37003433, 766239, 766234, 766231, 766233, 766232, 766238, 724906, 724907, 35895192, 36388974, 42796343, 35895190, 35895191, 35891484, 35891695, 36391504, 42796198, 1219271, 42797615, 42797616, 829421, 2041351575, 2042141476, 2042126947, 2042117210, 2041379169, 2042067388, 2039314825, 2042084532, 2042118832, 2041378979, 2041377069, 2039142356)
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets codesets on ((de.drug_source_concept_id = codesets.concept_id and codesets.codeset_id = 0))
) C
...
(
  select o.* 
  FROM @cdm_database_schema.OBSERVATION o
JOIN #Codesets codesets on ((o.observation_source_concept_id = codesets.concept_id and codesets.codeset_id = 0))
) C
